...
@onready
var effective_damage: Label = $Control/vBoxContainer/StatsContainer/StatLabels/EffectiveDamage
...
func set_game_over_stats(stats_component: StatisticsComponent) -> void:
	...
	damage_output.text = str(stats_component.total_effective_damage_output) + " / " + str(stats_component.total_damage_output)
